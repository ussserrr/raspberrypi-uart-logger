"""
Various BCD (binary-coded decimal) routines
"""
...
def int_to_bcd_bytes(val: int) -> bytes:
    ...
    val_as_str = str(val)
    # To be a multiple of 2, pad the string on the left with zeros
...
def bcd_to_int(val: int) -> int:
    ...
    # To be a multiple of 4, pad the string on the left with zeros
    while len(val_as_str) % 4 > 0:
        val_as_str = '0' + val_as_str  # '0101 0011 0101'

    out_as_str = ''
    # Iterate over the tetrades
